...
export default function CaregiverDashboard() {
      ...
      try {
        ...
        if (myProfile) {
          ...
          const bookingsRes = await api.get(
            `/bookings?caregiverId=${myProfile.id}`,
          );
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      // Refetch all bookings to get updated data
      const caregiversRes = await api.get("/caregivers");
      const allCaregivers = caregiversRes.data.data;
      const myProfile = allCaregivers.find(
        (cg: { user: { fullName: string }; id: string }) =>
          cg.user.fullName === user?.fullName,
      );
            ...
            {bookings.length === 0 ? (
              <Card>
                <p className="text-sm text-gray-400 text-center py-4">
                  No bookings yet. Make sure your profile is verified and you're
                  online.
                </p>
              </Card>
            ) : (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        ...
                        variant={
                          getStatusVariant(booking.status) as
                            | "green"
                            | "red"
                            | "gray"
                            | "yellow"
